...
The catalogue below is current through release 4 and class 23.
...
| 23 | Signed T3 scorer omits its prompt assets | `.governance/t3/score_t3.py score --project /data/s253_rea` exits before scoring with `Error: Sonnet prompt not found at /data/s253_rea/.governance/research/artifacts/t3_sonnet_scoring_prompt.md`; the fresh Moonshots source tree also lacks `research/artifacts/t3_*prompt.md`, so the installed signed scorer can verify while its required prompts are absent | s253 (REA Branch-C gate-minus-one recovery, standalone T3 run under Rex's 2026-09-25 API authorization; credentials SET, raw exit 1) | open; repair requires a signed source route that either enrolls the T3 prompt assets as managed members or changes the scorer to resolve an authenticated prompt source. Do not copy prompts into REA ad hoc; that would leave the signed scorer's dependency outside the managed population |
