...
CANONICAL_WRAPPER=$CANONICAL_ROOT/.github/write-enforcement/s152_successor_dynamic_approval_checked_wrapper.sh
CANONICAL_HELPER=$CANONICAL_ROOT/.github/write-enforcement/s152_successor_approval_resume.py
DEPLOYED_WRAPPER=/home/azureuser/.local/libexec/rea_enforcement/s152_successor_dynamic_approval_checked_wrapper.sh
DEPLOYED_HELPER=/home/azureuser/.local/libexec/rea_enforcement/s152_successor_approval_resume.py

refuse() {
    printf 'REFUSE(S152_SUCCESSOR_APPROVAL_WRAPPER): %s\n' "$1" >&2
...
git -C "$CANONICAL_ROOT" diff --quiet HEAD -- \
    .github/write-enforcement/s152_successor_dynamic_approval_checked_wrapper.sh \
    .github/write-enforcement/s152_successor_approval_resume.py \
...
export REA_S152_CHECKED_WRAPPER=rea-s152-public-successor-dynamic-approval-v2
case "${1-}" in
    "") exec /usr/bin/python3 "$DEPLOYED_HELPER" ;;
    --preflight) exec /usr/bin/python3 "$DEPLOYED_HELPER" --preflight ;;
