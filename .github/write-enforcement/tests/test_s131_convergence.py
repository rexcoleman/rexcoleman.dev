...
def test_member_population_is_complete_and_two_method_count():
    ...
    assert successor["artifact-integrity-authority-resolver"] == (
        "govML", "templates/build/enforcement/artifact_integrity_authority_resolver.py"
    )
    assert successor["artifact-integrity-production-authorities"] == (
    ...
    assert terminal_selector == "research_runtime_dependency_successor_members"
    terminal = contract.research_runtime_dependency_successor_members()
    assert len(terminal) == 297
...
def _run_five_root_builder(
    ...
    if terminal:
        arguments.append("--research-runtime-dependency-successor")
...
def test_exact_five_candidate_roots_close_installed_runtime_population(
    ...
    assert len(json.loads(honest.read_bytes())["members"]) == len(current) == 297
