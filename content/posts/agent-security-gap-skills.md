...
**SkillVet** (a tool we're designing) **— behavioral sandbox scanning.** The idea: instead of pattern-matching skill source code, run skills in an instrumented sandbox and observes their behavior across multiple heartbeat cycles. It watches for data exfiltration patterns, memory modification, credential access, and anomalous network activity. The goal: catch the skills that static analysis misses because their code is clean but their behavior is hostile.

**AgentArmor** (a tool we're designing) **— runtime behavioral monitoring.** SkillVet would catch threats pre-install. AgentArmor would catch them post-install — monitoring what skills actually do during live agent operation — file access patterns, API calls, network traffic, memory modifications — and alerts on behavioral anomalies. This is the EDR equivalent for agent skill security.
