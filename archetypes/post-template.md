---
title: ""
date: {{ .Date }}
draft: true
tags: []
format: ""  # technical-blog | tutorial | perspective | til | research-report
audience_side: ""  # of-ai | from-ai | both
image_count: 0  # R26: technical-blog ≥1, tutorial ≥1, research-report ≥3
description: ""
cover:
    image: ""
    hidden: true
---

<!-- R25 Pre-Publication Checklist:
  [ ] format: tag set
  [ ] audience_side: tag set
  [ ] image_count ≥ minimum for format (R26)
  [ ] Limitations section present (required for technical-blog, tutorial)
  [ ] What's Next or What I'm Doing About It section present
  [ ] CTA block at end (subscribe link)
  [ ] Standard bio footer
  [ ] Honesty check: all career claims verified against brand honesty constraint
  [ ] Forward reference to at least one other post
  [ ] All inline images have alt text
-->

<!-- HOOK: 1-2 sentences with a specific number or surprising finding -->



<!-- BODY: Structure depends on format:

  technical-blog: What I Built → What I Found → Why It Matters
  tutorial: Problem → Prerequisites → Steps (numbered) → Verification
  perspective: Thesis → Evidence (3-5 data points) → Implication
  til: What I found → Why it matters → Link
  research-report: Abstract → Hypothesis → Methodology → Data → Results → Analysis
-->



<!-- LIMITATIONS (required for technical-blog, tutorial, research-report) -->

### Limitations



<!-- WHAT'S NEXT (technical-blog, tutorial) or WHAT I'M DOING ABOUT IT (perspective) -->

### What's Next



---

*Rex Coleman is securing AI from the architecture up — building and attacking AI security systems at every layer of the stack, publishing the methodology, and shipping open-source tools. [rexcoleman.dev](https://rexcoleman.dev) · [GitHub](https://github.com/rexcoleman) · [Singularity Cybersecurity](https://singularitycyber.com)*

---

*If this was useful, [subscribe on Substack](https://substack.com/@rexcoleman) for weekly AI security research — findings, tools, and curated signal.*
